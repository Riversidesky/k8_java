...
//			}
		...
		public static void printSubjectStats(Student3[] students) {
	        ...
	        // 각 과목의 최대 및 최소 점수 및 해당 학생 초기화
	        int[] maxScores = new int[numSubjects];
	        int[] minScores = new int[numSubjects];
	        String[] maxScoreStudents = new String[numSubjects];
	        String[] minScoreStudents = new String[numSubjects];

	        // 각 과목별 최대, 최소 점수를 학생의 첫 번째 점수로 초기화
	        for (int i = 0; i < numSubjects; i++) {
	            maxScores[i] = students[0].scores[i];
	            minScores[i] = students[0].scores[i];
	            maxScoreStudents[i] = students[0].name;
	            minScoreStudents[i] = students[0].name;
	        }
	        ...
	        // 결과 출력
	        System.out.println("과목\t최대점수(학생)\t최소점수(학생)");
	        for (int i = 0; i < numSubjects; i++) {
	            String subject = students[0].subjects[i];
	            System.out.printf("%s\t%d(%s)\t%d(%s)\n", subject, maxScores[i], maxScoreStudents[i], minScores[i], minScoreStudents[i]);
	        }
	    }
