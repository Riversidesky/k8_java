...
public class train_실습2_14_1객체배열정렬 {
	...
	static void sortData(PhyscData d[]) {//객체 배열을 이름 순서로 정렬, 이름이 같으면 키 값을 정렬, 키 값이 같으면 시력으로 
		//compareTo()를 사용하여 구현
	}
	static int binarySearch(PhyscData d[], String s) {
	...
	static void showData(String s, PhyscData d[]) {
		System.out.print(s);
		
	}
	static PhyscData[] insertObject(PhyscData[] d, PhyscData nd){//배열의 사이즈를 1개 증가시킨후 insert되는 스트링 보다 큰 값들은 우측으로 이동, 사이즈가 증가된 스트링 배열을 리턴
		return null;
	}
