 ...
 * class CircularQueue1의 필드는 QUEUE_SIZE, que,	front, rear, isEmptyTag 변수만 사용
 */
...
class Point6 {
	...
	public Point6(int rndx, int rndy) {
		ix = rndx;
		iy = rndy;
	}
}
...
class CircularQueue1 {
	...
	public CircularQueue1(int count) {
		front = rear = 0;
		que = new Point6[count];
		isEmptyTag = true;
		//다음 2개 field 가 필요한지 확인 필요 
		QUEUE_SIZE = count;
	}
	void push(Point6 it) throws OverflowQueueException{ //rear값 다시쓰기
		if(isFull()) {
	...
	Point6 pop() throws EmptyQueueException{ //front값 다시쓰기
		if(isEmpty()) {
	...
	//--- 원형 큐가 가득 찼는가? --- 수정 필요//
	public boolean isFull() { //다시쓰기
		//추가
		return rear==QUEUE_SIZE && !isEmptyTag;
	}
	...
	public Point6 peek() throws EmptyQueueException {
		if (isEmpty())
...
public class 원형큐다시 {
	public static void main(String[] args) {
		...
		CircularQueue1 oq = new CircularQueue1(4); // 최대 4개를 인큐할 수 있는 큐
		...
		while (true) {
			...
			case 1: // 인큐
				...
				p = new Point6(rndx,rndy);
				try {
					oq.push(p);
					System.out.println("push: size() = "+ oq.size());
				} catch(CircularQueue1.OverflowQueueException e) {
			...
			case 2: // 디큐
				...
				} catch (CircularQueue1.EmptyQueueException e) {
